<div class="fixed-sm-200 fixed-lg-250 pull-sm-left">
	<div class="panel">
		...
		<div class="text-center pad-all bord-btm">
			<div class="pad-ver">				
				<div style="background-image: url(<?= base_url()?>uploads/profile_image/<?php echo $image[0]['thumb']?>); 
							background-size: cover; 
							margin: auto;" class="img-lg img-border img-circle"></div>
			</div>
			<h4 class="text-lg text-overflow mar-no"><?php echo $value->first_name." ".$value->last_name?></h4>
			<p class="text-sm">
				<?php 
					if(!empty($education_and_career[0]['occupation'])){
						echo $education_and_career[0]['occupation'];
					}
				?>
			</p>
			<p class="text-sm">
				<?php 
					if( !empty($present_address[0]['state']) ){
						echo $this->Crud_model->get_type_name_by_id('state', $present_address[0]['state']);
					}
				?>, 
				<?php 
					if( !empty($present_address[0]['country']) ){
						echo $this->Crud_model->get_type_name_by_id('country', $present_address[0]['country']);
					}
				?>
			</p>
