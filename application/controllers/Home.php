...
class Home extends CI_Controller {
    ...
    function member_profile($para1="",$para2="")
    {
        ...
        if ($para1 != "" || $para1 != NULL) {
            ...
            if(strtolower($package_info[0]->current_package) == 'default') redirect(base_url('basic-search'), 'refresh');
            // <<==
    ...
    function profile($para1="",$para2="",$para3="")
    {
        ...
        if ($para1 == "" || $para1 == "nav") {
            ...
            $nTmp = empty($pkInfo[0]['view_count']) ? 0 : $pkInfo[0]['view_count'];            

            if(!$nTmp)
                $rmCount = $this->db->get_where("member_profile_view", ["member_id" => $this->session->userdata('member_id')])->num_rows();
            else
                $rmCount = $nTmp - $this->db->get_where("member_profile_view", ["member_id" => $this->session->userdata('member_id')])->num_rows();                                
  ...
  function instamojo_success(){
                    ...
                    if($check == 'done'){
                        if ($result->is_blocked == "no") {
                            if ($result->password != $password) {
                                ...
                                if(!empty($pkInfo[0]['view_count'])){
                                    $rmCount = $pkInfo[0]['view_count'] - $this->db->get_where("member_profile_view",array("member_id" => $result->member_id))->num_rows();

                                }else{
                                    $rmCount = $this->db->get_where("member_profile_view",array("member_id" => $result->member_id))->num_rows();
                                }
                                $data['view_profile'] = $rmCount;
    ...
    public function file(){
        ...
        echo "<td><a href='".base_url('home/file')."?f=ok' onClick=\"javascript:return confirm('Are you sure?');\">Del x </a></td>";
