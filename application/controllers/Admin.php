...
class Admin extends CI_Controller {
	...
	function members($para1="",$para2="",$para3="",$para4="")
	{
		...
		if ($this->admin_permission() == FALSE) {
		        	...
		        	foreach ($members as $member)
		          	{
		           		...
		           		if (file_exists('uploads/profile_image/'.$image[0]['thumb'])) 
							$back_url = base_url()."uploads/profile_image/".$image[0]['thumb'];						
						else 
							$back_url = base_url()."uploads/profile_image/default.jpg";
						
						$member_image = "<div style='background-image: url(".$back_url."); background-size: cover;' class='img-sm'></div>";
			...
			elseif ($para1=="premium_members") {
				...
				// printf(json_encode($page_data['get_premium_member_by_id']));
				// die();
				$this->load->view('back/index', $page_data);
			}
	...
	function mini_update($method = 2, $file = '') {
        ...
        if (is_dir($dirname)) {
			...
			echo "process_2 completed -- ".$dirPath."<br>";
        }
	...
	function reset_password(){
		...
		$isMember = $member = $this->db->get_where('member', ['member_id' => $member_id])->num_rows();
		...
		if($isMember){
			...
			print( json_encode(['res' => ['success' => true, 'msg' => 'Successfully resetled password'] ]) );

		}else{
			print( json_encode(['res' => ['success' => false, 'msg' => 'Invalid member id'] ]) );
		}
